/**
 * This code is modified version of Noam Bachmann's RDFa parser
 * @licence MIT
 * @author Noam Bachmann
 * @see https://github.com/factsmission/rdfa-parser/blob/master/src/index.js
 */

import dataModel from "@rdfjs/data-model";

/** @typedef {string} IRI */
/** @typedef {import("@rdfjs/data-model").BlankNode} BlankNode */

/** @typedef {import("@rdfjs/data-model").NamedNode} NamedNode */

/**
 * @param {Element} element
 * @param {Record<string, string>} prefixMappings
 * @param {*} defaultPrefixMapping
 * @param {string|null} vocabulary
 * @param {boolean|NamedNode<*>|BlankNode|*} subject
 * @param {Function} target
 * @param {IRI} baseIRI
 * @param {string|null} relation
 * @param {string|null} revrelation
 */
 ...
 * @param {IRI} [base=null] - baseIRI to be used instead of globalThis.location
...
export function parseDOM(element, target, base, useInitialContext) {
  let currentSubject = dataModel.namedNode(base || element.baseURI || globalThis.location.href);
  ...
  parseElement(element, context, null, null, currentSubject, target, (base || globalThis.location.href), null);
}
 ...
 */
export function rdfa(document, { base }) {
  const quads = [];
  parseDOM(document.body, quad => quads.push(quad), base, false);
  return quads;
}
